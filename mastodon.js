class Mastodon {
    ...
    async get_servers() {
        const response = await fetch(`https://api.fedidb.org/v1/servers?limit=40`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
    ...
    // Not currently used, would return the current platform a specific account is hosted on
    // Not part of the current Mastodon API
    async get_software(severName) {
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");

        var graphql = JSON.stringify({
        query: `{node(domain: "${serverName}") { softwarename } }`,
        variables: {}
        });
        ...
        const response = await fetch("https://api.fediverse.observer/", requestOptions);
