...
var app = Vue.createApp({
    ...
    methods: {
        ...
        async get_followed(userId, forceRefresh = false, readOnly = true) {
            ...
            if (this.$root.my_account.following_count <= this.$root.followed_threshold && this.$root.my_account.following_count > 0) {
        ...
        async search(searchValue, type = "accounts", accountId = 0, followedOnly = true, limit = 40) {
            // Using /v2/search 
            ...
            var response = await this.mastodon.get(`/api/v2/search?q=${searchValue}&type=${type}&following=${followedOnly}&limit=${this.max_retrieval_count}`);
            data = await response.json();
            ...
            if (!response.ok) {
                ...
                if (data.accounts.length == this.max_retrieval_count) {
                    ...
                    while (newData.accounts.length == this.max_retrieval_count) {
                        next = next++;
                        const response = await this.mastodon.get(`/api/v2/search?q=${searchValue}&type=account&limit=${this.max_retrieval_count}&offset=${next}`);
                        const newData = await response.json();
